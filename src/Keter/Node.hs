...
setupNode' knc = do 
  createDirectory True (knCfgRoot knc)
...
spawnNode knw kn knargs = isKeterNode knw kn >>= (\ekn -> case ekn of 
                                                           (Left e) ->  return $ Left e
                                                           (Right kn) -> do 
                                                             writeFile "test.this.blows" "here is a stupid test"
                                                             tt <- TempFolder.unpackTempTar 
                                                                         Nothing 
                                                                        
                                                                         (knmAppTemp knw)
                                                                         (nodeTypeFilePath </> (unKeterNode kn ))
                                                                         (show nodeTypeFilePath)
                                                                         buildKeterNode
                                                             return $ Right () 
                                                 )
